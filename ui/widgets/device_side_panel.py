...
功能及相关说明:
  ...
  LAN 在线成员只作为局域网实时连接摘要，不写入设备主表，不替代 Verify 设备真相源。
  不承载远控/投屏入口。
"""
...
class DeviceSidePanel(QWidget):
    ...
    def __init__(self, parent: QWidget | None = None) -> None:
        ...
        self._lan_empty_label: QLabel | None = None
        self._build()
    ...
    def _build(self) -> None:
        ...
        self._lan_empty_label = self._hint("暂无 LAN 在线成员")
        self._lan_list_layout.addWidget(self._lan_empty_label)
    ...
    def update_lan_members(self, members: list["TeamMember"], verify_devices: list[DeviceInfo] | None = None) -> None:
        ...
        verify_devices = verify_devices or []
        verify_ids = {
            value
            for dev in verify_devices
            for value in (dev.device_id, dev.display_id, dev.connection_label, dev.device_fingerprint)
            if value
        }
        ...
        for member in sorted(members, key=lambda item: item.device_id):
            matched = member.device_id in verify_ids
    ...
    def _lan_member_card(self, member: "TeamMember", matched: bool) -> QWidget:
        ...
        top.addStretch()
        match = QLabel("已匹配" if matched else "未匹配")
        match.setStyleSheet(
